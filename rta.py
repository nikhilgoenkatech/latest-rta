...
LABS_REGISTER_URL = config['labs_register_api']
API_CODE_LAB_DELETE=config['labs_delete_api']
# Actions Control
...
ACTION_DELETE_CODE_LABS = config['action_remove']['delete_code_labs']
...
ACTION_CREATE_CODE_LABS = config['action_create']['setup_code_labs']
print ("ACTION_CREATE_CODE_LABS " + str(ACTION_CREATE_CODE_LABS))
'''
...
SKEL_CODE_LABS = json.load(open('scenario/code_labs.json','r'))
...
def do_code_labs_post(data):
    logging.debug("Entering do_code_labs_post")
    response = requests.post(LABS_REGISTER_URL, json=data, headers=getHeader(), verify=verifyRequest())
    logging.debug('POST Reponse content: ' +
...
def do_code_labs_delete(userId):
    print(API_CODE_LAB_DELETE + userId)
    response = requests.delete(
        API_CODE_LAB_DELETE + userId, headers=getHeader(), verify=verifyRequest())
    logging.debug('DELETE Reponse content: ' +
                  str(response.content))
    return response
...
def do_d1p_acm():
    for id in CSV_DATA:
        ...
        cmds = []
        cmds.append(['cd /home/ubuntu/', False])
        cmds.append(['git clone https://github.com/nikhilgoenkatech/ACMD1Workshops.git', True])
        cmds.append(['cd /home/ubuntu/ACMD1Workshops', True])
        cmds.append(['./install-workshop-tech-stack.sh', True])
        cmds.append(['docker ps -a', True])
        cmds.append(['ls -lrt /home/ubuntu/ACMD1Workshops/', True])
...
def setup_code_labs(data):
    print("setup_code_labs")
    if not ACTION_CREATE_CODE_LABS:
        return True

    post_data = copy.deepcopy(SKEL_CODE_LABS)
    post_data['firstName'] = data[key_firstName]
    post_data['lastName'] = data[key_lastName]
    post_data['email'] = data['email']

    email = data['email']
    if "keptn" in email:
        post_data['steps'] = ['5fa201b5cf4917273cab8fe4']
    try:
      # Load client
      post_data['instanceId'] = data['instanceId']
    except: 
      post_data['instanceId'] = ""

    try:
       post_data['tenant'] = data[key_tenantUrl]
    except KeyError:
       post_data['tenant'] = "" 
    except:
       post_data['tenant'] = ""
    ...
    print(post_data)
    response = do_code_labs_post(post_data)
    print (response.text)
...
def delete_code_labs(data):
    if not ACTION_DELETE_CODE_LABS:
        return True
    userId = data['email']
    response = do_code_labs_delete(userId)
    return validate_set_action_status(response, data, 'delete_code_labs')
...
def execute_ssh_command(data, cmds, as_sudo=False):
    ...
    try:
        ...
        print("in execute_ssh_command")
        sshUser = data['sshUser']
        sshPassword = data['sshPassword']
        print("in execute_ssh_command", sshUser, " " , sshPassword, " " , ipdns)
        ssh = shClient(ipdns, port=22, username=sshUser, password=sshPassword)
        retval = 0
        for cmd in cmds:
            print("Comamnds")
            c = cmd[0]
...
def main():
    try:
        ...
        if len(sys.argv) >= 2:
            ...
            elif command == 'd1p-acm':
                load_users_csv()
                logging.info("====== D1P ACM called ======")
                do_d1p_acm()
                saveResults = True
